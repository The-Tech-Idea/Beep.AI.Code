"""JavaScript code intelligence tools using tree-sitter parsing."""
...
class JavaScriptSymbolsTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "Search JavaScript/JSX symbols (functions, classes, imports) using tree-sitter parsing."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "kind": {
                "type": "string",
                "description": "Filter by symbol kind: function, class, import (optional).",
            },
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        output_lines = [f"JavaScript symbols for query: {query!r} (found: {len(filtered)})"]
...
class JavaScriptDefinitionsTool(BaseTool):
    def __init__(self, workspace_root: Path) -> None:
        self._workspace_root = workspace_root
        from beep.app_service import get_app_service
    ...
    @property
    def description(self) -> str:
        return "Find JavaScript function/class definitions in a specific file using tree-sitter parsing."
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            "file_path": {
                "type": "string",
                "description": "JavaScript file path inside the workspace.",
            },
        }
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        output_lines = [f"JavaScript definitions in {file_path} (found: {len(defs)})"]
        for sym in defs:
...
def build_javascript_intelligence_tools(
    *,
    workspace_root: Path,
) -> tuple[BaseTool, ...]:
    return (
        JavaScriptSymbolsTool(workspace_root),
        JavaScriptDefinitionsTool(workspace_root),
    )
