...
| PH-23 | [~] | `.plans/PHASE_23_SERVER_INTEGRATION_PARITY.md` | In progress (P0 — Rank 0): Slices 1-4 complete (namespace migration, endpoint centralization, coding-session via unified chat, capability discovery, default-backend UX). Slices 5-7 blocked on server access. Backing doc: `docs/features/server-integration.md`. |
| PH-20 | [x] | `.plans/PHASE_20_INTEGRATIONS_CATALOG.md` | Complete: all 7 slices done — catalog models, curated registry (24 entries), governance/audit, installer with skill materialization, tool detector wired into diagnostics, AppService IntegrationsRegistry, `beep skills` and `beep catalog` CLI commands, expanded MCP presets (13 servers). 74 tests, ruff clean. |
| PH-21 | [~] | `.plans/PHASE_21_CODING_AGENT_PARITY_V2.md` | In progress (Rank 2): 6/7 slices complete. Slice 1 (Plan/Build toggle), Slice 2 (@mention subagents), Slice 4 (parallel worktree agents + fanout CLI), Slice 5 (generic LSP client + 18-language registry), Slice 6 (LiteLLM provider + 8 built-ins), Slice 7 polish (edit checkpoint timeline + /undoedit, /checkpoints commands, model-specific cost guardrails with price hints). 150 tests passing. Remaining: Slice 3 (persistent agent server — needs FastAPI). |
| PH-22 | [~] | `.plans/PHASE_22_GREENFIELD_COMMANDS.md` | In progress (Rank 3): 3/4 slices complete. Slice 1 (web search) — WebSearchService with BEEP_WEBSEARCH=1 gate, WebSearchTool registered read-only safe in factory, /search slash command, beep search CLI command, 9 tests. Slice 2 (git family) — GitWorkflow service + beep commit/branch/diff/vcs-status, 11 tests. Slice 3 (beep init) — ProjectScaffold + beep init --force, 8 tests. 28 tests across PH-22. Slice 4 (docs) remains. |
...
| P0-6 | [x] `docs/` suite: INDEX, APP_OVERVIEW, CLI reference, SERVICES_REGISTRY, ENHANCEMENT_PLAN, `docs/features/*` per service                  | `docs/INDEX.md` links valid |


**Primary files:** `README.md`, `docs/**`, `beep/cli.py`, `beep/commands/diagnostics.py`
